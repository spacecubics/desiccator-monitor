...
class SwitchbotMeterPlugin():
    ...
    def fetch(self):
        scanner = Scanner(self.params['hcidev']).withDelegate(
            SwitchbotScanDelegate(self.params['macaddrs']))
        try:
            scanner.scan(self.params['scantimeout'])
        except Exception as e:
            subprocess.call(
                f'btmgmt --index {self.params["hcidev"]} power off && btmgmt --index {self.params["hcidev"]} power on', shell=True)
            scanner.scan(self.params['scantimeout'])
